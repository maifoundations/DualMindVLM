...
class MathEvaluator():
    ...
    def compute_accuracy(self, answer_content, gt, options, question_type):
        option = chr(65 + int(gt))
        content = options[ord(option) - 65]
        ...
        if float(verify(answer, gt_content)) > 0:
            return 1.0
        if content.lower().strip() == answer_content.lower().strip():
            return 1.0
        if self.num_to_word.get(content.strip().lower(), "xxxx") == answer_content.strip().lower():
            return 1.0
    ...
    def evaluate(self, results: Dict) -> float:
        ...
        print(f"Average Short Thinking Length: {short_response_length / short_num if short_num > 0 else 0}, Average Long Thinking Length: {long_response_length / long_num if long_num > 0 else 0}")
...
if __name__ == "__main__":
    ...
    evaluator = MathEvaluator(dataset_name='scienceqa')

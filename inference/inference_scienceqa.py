...
def inference_chat_model(dataset_name, out_dir, seed, system_prompt):
    ...
    sampling_params = SamplingParams(temperature=0.01, top_p=0.001, top_k=1, max_tokens=1024,
    ...
    for idx, (data_item, model_output) in enumerate(zip(dataset, model_outputs)):
        ...
        outputs[idx]['question_type'] = "multi_choices"
        outputs[idx]['options'] = data_item['choices']
...
4. The final answer MUST BE put in \\boxed{}."""
    ...
    inference_chat_model("scienceqa", args.out_dir, args.seed, SYSTEM_PROMPT)
